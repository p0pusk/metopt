...
class Plot:
    ...
    def __init__(self, surface, r, w):
        ...
        self.r = r * self.scale
        self.w = w * self.scale
        self.h = 2 * math.sqrt((self.scale + self.r) ** 2 - self.w**2)
        self.degree = 0

        self.ul: tuple[float, float] = (self.init_x + self.h, self.init_y)
        self.ur: tuple[float, float] = (self.init_x + self.h, self.init_y + self.w)
        self.bl: tuple[float, float] = (self.init_x, self.init_y)
        self.br: tuple[float, float] = (self.init_x, self.init_y + self.w)
